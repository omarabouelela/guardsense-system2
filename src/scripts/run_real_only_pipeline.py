"""One-command orchestration for the real-only binary Trigger/Verifier pipeline with Live Frigate Integration."""
...
import cv2  # Required for pulling the live Frigate restream
import yaml
...
def _parse_args() -> argparse.Namespace:
    ...
    # New: Live Streaming Analytics Integration
    parser.add_argument("--live", action="store_true", help="Run continuous live inference using Frigate's RTSP feed")
    parser.add_argument("--stream-url", default="rtsp://127.0.0.1:8554/classroom_cam", help="Frigate go2rtc high-speed local stream mirror")
    ...
    # Configuration Files
    parser.add_argument("--pose-config", type=Path, default=Path("D:\guardsense-system2-real-only-real-only-v1\configs\pose_extraction.yaml"), help="Pose extraction config")
    parser.add_argument("--trigger-data-config", type=Path, default=Path("D:\guardsense-system2-real-only-real-only-v1\configs/trigger_data.yaml"), help="Trigger preprocessing config")
    parser.add_argument("--trigger-train-config", type=Path, default=Path("D:\guardsense-system2-real-only-real-only-v1\configs/trigger_train.yaml"), help="Trigger training config")
    parser.add_argument("--trigger-eval-config", type=Path, default=Path("D:\guardsense-system2-real-only-real-only-v1\configs/trigger_eval.yaml"), help="Trigger evaluation config")
    parser.add_argument("--verifier-data-config", type=Path, default=Path("D:\guardsense-system2-real-only-real-only-v1\configs/verifier_data.yaml"), help="Verifier preprocessing config")
    parser.add_argument("--verifier-train-config", type=Path, default=Path("D:\guardsense-system2-real-only-real-only-v1\configs/verifier_train.yaml"), help="Verifier training config")
    parser.add_argument("--verifier-eval-config", type=Path, default=Path("D:\guardsense-system2-real-only-real-only-v1\configs/verifier_eval.yaml"), help="Verifier evaluation config")
...
def _run_live_stream_pipeline(args: argparse.Namespace) -> int:
    """Hooks directly into Frigate's RTSP feed and executes Trigger/Verifier inference sequentially."""
    LOGGER.info("Starting Live Inference Mode. Initializing models...")
    
    # 1. Fetch paths for latest model checkpoints using your existing layout
    trigger_dir = _load_yaml(args.trigger_train_config).get("output_dir", "artifacts/runs")
    verifier_dir = _load_yaml(args.verifier_train_config).get("output_dir", "artifacts/verifier_runs")
    
    latest_trigger = latest_checkpoint(base_dir=Path(trigger_dir), run_prefix="trigger_")
    latest_verifier = latest_checkpoint(base_dir=Path(verifier_dir), run_prefix="verifier_")
    
    LOGGER.info("Loaded Trigger Model Checkpoint: %s", latest_trigger)
    LOGGER.info("Loaded Verifier Model Checkpoint: %s", latest_verifier)
    
    # [Runtime Initialization Note]
    # In your production runtime, import your inference wrapper classes here:
    # from src.models import RealTimePoseExtractor, DualStageInferenceEngine
    # engine = DualStageInferenceEngine(trigger_path=latest_trigger, verifier_path=latest_verifier)

    # 2. Open high-speed feed connection from Frigate go2rtc restreamer
    cap = cv2.VideoCapture(args.stream_url)
    if not cap.isOpened():
        LOGGER.error("Could not link to Frigate live stream at: %s", args.stream_url)
        return 1

    LOGGER.info("Successfully connected to Frigate! Streaming live pipeline...")
    
    try:
        while True:
            ret, frame = cap.read()
            if not ret:
                LOGGER.warning("Dropped frame or stream paused. Retrying...")
                time.sleep(0.5)
                continue
            
            # --- STAGE 2A: Continuous Live Pose Extraction ---
            # extract_pose_script code logic executes live frame-by-frame here
            # current_poses = RealTimePoseExtractor.extract(frame)
            
            # --- STAGE 2B: Binary Trigger Analysis Loop ---
            # is_triggered = engine.predict_trigger(current_poses)
            is_triggered = True  # Simulating active pass-through
            
            if is_triggered:
                # --- STAGE 2C: High-Fidelity Verifier Pipeline ---
                # Only runs validation verification if stage 1 passes
                # final_prediction = engine.predict_verifier(current_poses)
                pass

            # Slow iteration speed to match Frigate's config profile detect FPS (5 FPS = 0.2s sleep)
            time.sleep(0.2)
            
    except KeyboardInterrupt:
        LOGGER.info("Live pipeline execution interrupted by user.")
    finally:
        cap.release()
...
def main() -> int:
    ...
    # If running live stream integration, we bypass static configuration verification checks
    if args.live:
        return _run_live_stream_pipeline(args)
    ...
    stages = [
        Stage("extract_pose", "Extract Trigger Pose", Path("D:\guardsense-system2-real-only-real-only-v1\src\scripts\extract_pose_from_videos.py"), "pose_config", selected["extract_pose"]),
        Stage("prepare_trigger", "Prepare Trigger Dataset", Path("D:\guardsense-system2-real-only-real-only-v1\src\scripts\prepare_trigger_data.py"), "trigger_data_config", selected["prepare_trigger"]),
        Stage("train_trigger", "Train Trigger", Path("D:\guardsense-system2-real-only-real-only-v1\src\scripts\train_trigger.py"), "trigger_train_config", selected["train_trigger"]),
        Stage("eval_trigger", "Evaluate Trigger", Path("D:\guardsense-system2-real-only-real-only-v1\src\scripts\eval_trigger.py"), "trigger_eval_config", selected["eval_trigger"]),
        Stage(
            "prepare_verifier",
        ...
        Stage("train_verifier", "Train Verifier", Path("D:\guardsense-system2-real-only-real-only-v1\src\scripts\train_verifier.py"), "verifier_train_config", selected["train_verifier"]),
        Stage("eval_verifier", "Evaluate Verifier", Path("D:\guardsense-system2-real-only-real-only-v1\src\scripts\eval_verifier.py"), "verifier_eval_config", selected["eval_verifier"]),
    ]
    ...
    for idx, stage in enumerate(stages, start=1):
        ...
        if return_code != 0:
            ...
            results.append(StageResult(stage.key, status="failed", command=rendered_cmd, message=message))
